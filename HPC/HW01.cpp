...
static void betterSearch(const AlignedIntArray &hayStack, const AlignedIntArray & needles, AlignedIntArray &indices, StackAllocator & allocator) {
  ...
  const int needlesSize = needles.getCount();
  const int numKeys = needlesSize > 1024 && haystackSize > 2048 ? 512 : 64;  // Adjustable size of keys
  int *keys = allocator.alloc<int>(numKeys);  // int keys[numKeys];
  const int haystackPart = haystackSize / numKeys;
  for (int i = 0; i < numKeys; ++i) keys[i] = hayStack[i * haystackPart];

  const int numKeysQuarter = numKeys >> 2;
  // Some fast search constants
  const int haystackMin = keys[0],
            haystack1_4 = keys[numKeysQuarter],
            haystack2_4 = keys[(numKeys >> 1)],
            haystack3_4 = keys[(numKeys >> 1) + numKeysQuarter],
            haystackMax = hayStack[haystackSize - 1];

  for (int c = 0; c < needlesSize; ++c) {
    const int value = needles[c];

    // If it can not be in the array
    if (value > haystackMax || value <= haystackMin) {
      if (value != haystackMin) {
        indices[c] = -1;
      } else {
        indices[c] = 0;
      }
    ...
    // SubSearch procedure - a binary search on keys
    int left, count;
    int index = (value > haystack1_4) + (value > haystack2_4) + (value > haystack3_4); // Fast compare
    int leftSS = index * numKeysQuarter;
    int rightSS = (index + 1) * numKeysQuarter;
    while (leftSS < rightSS) {
      index = (leftSS + rightSS) >> 1;
      if (keys[index] < value) {
        leftSS = index + 1;
      } else {
        rightSS = index;
    ...
    if (keys[index] >= value) --index;  // Lower bound part

    // Adjust before searching
    left = index * haystackPart;
    count = (index < numKeys - 1) ? haystackPart + 1 : haystackSize - left;
  ...
  allocator.freeAll();
  // On MSVS and GCC the algorithm is faster,
  // but for best results use MinGW on DevC++ :)
}
